...
## Weiner
#############################################
rand_n = rnorm(1000, 0, 1)
rand_n = cumsum(rand_n)
plot(rand_n, type= 'l')
...
sims <- simdiff(100,2,model = 'Q') #datos de simulaci??n3
modelo <- diffIRT(sims$rt, sims$x, model = 'Q')
summary(modelo)
plot(res$ai, coef(res)$item[, "ai"]) #reales ai vs estimados de los items
abline(0, 1)
resp_out <- RespFit(modelo, 1)
resp_out #Bondad de ajuste Mr, df y p

##ESTIMA PARAMETROS DE DIFUSION
data("rotation", package = "diffIRT")
x <- rotation[, 1:6]
rt <- rotation[, 11:16]
res <- diffIRT(rt, x, "Q", se = TRUE)
...
QQdiff(res, item = 1:3) #distribucion de los parametros
QQdiff(res, item = 4:6)
